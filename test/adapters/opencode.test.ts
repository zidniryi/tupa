...
try {
  ...
  test("opencode adapter (skipped: node:sqlite unavailable on this Node version)", () => {});
  process.exit(0);
}

const dir = mkdtempSync(join(tmpdir(), "tupa-opencode-test-"));
process.env.TUPA_OPENCODE_DIR = dir;
...
test("detect() finds the fixture opencode.db", async () => {
  assert.equal(await opencodeAdapter.detect(), true);
});

test("listSessions() reads title and updatedAt from the session table", async () => {
  const sessions = await opencodeAdapter.listSessions("/tmp/fixture-project");
  assert.equal(sessions.length, 1);
  const session = sessions[0]!;
  assert.equal(session.id, "ses_fixture1");
  assert.equal(session.tool, "opencode");
...
test("listSessions() on an unmatched directory returns no sessions", async () => {
  const sessions = await opencodeAdapter.listSessions("/tmp/does-not-exist");
  assert.deepEqual(sessions, []);
});

test("readSession() joins message + part rows and collects changed files", async () => {
  const transcript = await opencodeAdapter.readSession("ses_fixture1");
...
test("readSession() returns undefined for an unknown id", async () => {
  const transcript = await opencodeAdapter.readSession("does-not-exist");
  assert.equal(transcript, undefined);
});

test("resumeCommand() and launchWithContext() return shell commands", () => {
  assert.equal(opencodeAdapter.resumeCommand("ses_fixture1", "/tmp"), "opencode -s ses_fixture1");
  assert.match(opencodeAdapter.launchWithContext("/tmp/HANDOFF.md", "/tmp"), /Read \/tmp\/HANDOFF\.md and continue/);
});
